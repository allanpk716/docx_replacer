...
已完成的核心功能：Excel 两列/三列格式自动检测、关键词替换（正文+页眉+页脚）、富文本格式保留、批注追踪、审核清理（批注清除+内容控件解包）、CLI 模式（fill/cleanup/inspect）、在线更新（Velopack + 内网 Go 服务器 + GitHub Releases 双通道）、主界面紧凑化至 900x550。配置持久化已修复（~/.docx_replacer/update-config.json）。更新检测已修复：移除 ExplicitChannel 使 Velopack 正确查找 releases.win.json。
