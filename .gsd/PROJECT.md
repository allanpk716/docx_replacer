...
已完成的核心功能：Excel 两列/三列格式自动检测、关键词替换（正文+页眉+页脚）、富文本格式保留、批注追踪、审核清理（批注清除+内容控件解包）、CLI 模式（fill/cleanup/inspect）、在线更新（Velopack + 内网 Go 服务器 + GitHub Releases 双通道）、主界面紧凑化至 900x550。配置持久化存在 bug：update-config.json 放在 Velopack 安装目录内，安装/更新时被覆盖导致内网更新地址丢失。
...
- [x] M012-li0ip5: 主界面布局紧凑化与拖放修复 — 窗口从 1400x900 降至 900x550，GroupBox 全移除，拖放迁移到 TextBox AllowDrop，窗口未聚焦拖放通过 Window 级激激活修复
- [ ] M013-ueix00: 更新配置持久化路径修复 — 将 update-config.json 从 Velopack 安装目录迁移到 ~/.docx_replacer/，彻底隔离安装/更新生命周期
