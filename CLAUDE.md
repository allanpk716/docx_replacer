...
2. **ViewModel Architecture（协调器 + 子 ViewModel 模式）**:

   项目采用协调器（Coordinator）模式：`MainWindowViewModel` 作为主协调器持有子 ViewModel 引用，管理窗口级状态和命令分发。子 ViewModel 各自独立管理自己的 UI 状态和业务逻辑。

   | ViewModel | 位置 | 基类/模式 | 职责 |
   |-----------|------|-----------|------|
   | `MainWindowViewModel` | `ViewModels/` | 手写 INPC（`ObservableObject.cs`） | 主窗口协调器，持有子 VM 引用，管理填充/清理/更新等所有 Tab 的命令和状态 |
   | `CleanupViewModel` | `DocuFiller/ViewModels/` | 手写 INPC（`ObservableObject.cs`） | 文档清理功能（批注移除、控件解包），被 CleanupWindow 使用 |
   | `DownloadProgressViewModel` | `ViewModels/` | CommunityToolkit.Mvvm | 下载进度显示（进度百分比、速度、剩余时间、取消），被 DownloadProgressWindow 使用 |
   | `UpdateSettingsViewModel` | `ViewModels/` | CommunityToolkit.Mvvm | 更新设置编辑（更新源 URL、通道选择、保存/取消），被 UpdateSettingsWindow 使用 |

   **CommunityToolkit.Mvvm 使用规范**：
   - 新创建的 ViewModel 使用 CommunityToolkit.Mvvm 8.4 源代码生成器
   - 继承 `CommunityToolkit.Mvvm.ComponentModel.ObservableObject`（必须使用完全限定名，避免与项目自定义的 `ObservableObject.cs` 冲突）
   - 使用 `[ObservableProperty] private type _fieldName;` 声明绑定属性（下划线前缀自动生成 PascalCase 属性名）
   - 使用 `[RelayCommand]` 或 `[RelayCommand(CanExecute = nameof(CanX))]` 声明命令
   - 属性变更副作用使用 `partial void OnPropertyNameChanged(T value)` partial 方法
   - ViewModel 类必须标记为 `partial class`
   - 项目自定义的 `ObservableObject.cs` 和 `RelayCommand.cs` 保留用于 `MainWindowViewModel` 和 `CleanupViewModel` 的渐进式迁移
...
### Service Layer Architecture (11 个服务接口 + 2 个处理器)
...
| 更新管理 | `IUpdateService` | `UpdateService` | Velopack 更新管理（检查更新、下载、应用更新、源切换） |
...
| update 命令 | `UpdateCommand` | 检查并安装应用更新（--yes 自动下载重启） |
...
| OpenXML 工具 | `OpenXmlHelper` | OpenXML 内容控件共享工具方法（从 ContentControlProcessor 提取的公共逻辑） |

### DI 生命周期选择

| 生命周期 | 适用场景 | 服务示例 |
|----------|----------|----------|
| **Singleton** | 无状态服务、全局缓存、线程安全服务 | IFileService, IExcelDataParser, ITemplateCacheService, IUpdateService, ContentControlProcessor |
...
| `update` | success | update 命令输出（版本检查/下载进度/更新提醒） | `{currentVersion, latestVersion, hasUpdate, isInstalled, updateSourceType}` 或 `{reminder: true, latestVersion, message}` |
...
- `Utils/OpenXmlHelper.cs`: OpenXML 内容控件共享工具方法（表格单元格位置检测等）
...
├── MainWindow.xaml / .xaml.cs          # 主窗口（位于项目根目录）
├── DocuFiller.csproj                  # 项目文件
├── Configuration/                     # 配置类（AppSettings, LoggingSettings 等）
├── Models/                            # 数据模型
├── ViewModels/                        # 视图模型
│   ├── MainWindowViewModel.cs          # 主窗口 ViewModel（协调器，手写 INPC）
│   ├── DownloadProgressViewModel.cs    # 下载进度 ViewModel（CT.Mvvm）
│   ├── UpdateSettingsViewModel.cs      # 更新设置 ViewModel（CT.Mvvm）
│   ├── ObservableObject.cs             # MVVM 基类（手写，用于 MainWindowVM）
│   └── RelayCommand.cs                # MVVM 命令基类（手写，用于 MainWindowVM）
...
│       └── UpdateCommand.cs           # update: 版本更新检查与安装
├── Program.cs                         # 应用入口（CLI/GUI 双模式分发）
├── Services/                          # 业务服务
│   ├── Interfaces/                    # 11 个服务接口定义
│   └── UpdateService.cs              # Velopack 更新服务实现
├── Converters/                        # WPF 值转换器
├── Utils/                             # 工具类（OpenXmlHelper, ValidationHelper 等）
├── Exceptions/                        # 自定义异常
├── Resources/                         # 应用资源（app.ico, app.png）
├── DocuFiller/                        # WPF 资源字典和样式
│   ├── Services/                      # 资源相关服务
│   ├── Utils/                         # 资源工具
│   ├── ViewModels/                    # 资源 ViewModel
│   │   └── CleanupViewModel.cs        # 清理功能 ViewModel（手写 INPC）
│   └── Views/                         # 资源视图
│       ├── CleanupWindow.xaml          # 清理功能窗口
│       ├── UpdateSettingsWindow.xaml   # 更新设置窗口
│       └── DownloadProgressWindow.xaml # 下载进度窗口
├── update-server/                     # Go 更新服务器（内网版本分发）
│   ├── main.go                        # 服务器入口
│   ├── handler/                       # HTTP 请求处理器
