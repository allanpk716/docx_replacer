...
选择和配置数据源文件，支持 JSON 和 Excel 两种格式。

#### 页面元素

| 页面 | 区域 | 功能描述 |
|------|------|----------|
| 主界面 | 数据文件选择 | 文件选择按钮，支持 .json 和 .xlsx 格式 |
| 主界面 | 数据类型指示 | 自动识别并显示当前数据文件类型（"Excel (支持格式)" 或 "JSON (纯文本)"） |
| 主界面 | 数据预览 | 数据预览表格，显示字段映射关系 |
| 主界面 | 数据验证 | 验证状态指示器，显示数据格式校验结果 |

#### 3.2.1 JSON 数据格式

JSON 文件格式为数组，每个对象代表一组关键词-值对：
...
- **关键词格式**：直接使用字段名（如 `FieldA`），对应模板中内容控件的 Tag 值
- **数据验证**：验证 JSON 语法正确性、字段非空、格式合规
...
提供独立的清理窗口，用于去除 DocuFiller 生成的处理痕迹，使文档看起来为最终版本。

#### 页面元素

| 页面 | 区域 | 功能描述 |
|------|------|----------|
| 清理窗口 | 文件输入 | 拖拽选择单个 docx 文件或包含 docx 文件的文件夹 |
| 清理窗口 | 输出设置 | 设置清理后的输出目录 |
| 清理窗口 | 文件列表 | 显示待清理文件列表，每项显示文件名、大小和状态 |
| 清理窗口 | 处理控制 | 开始清理按钮、进度显示 |
| 清理窗口 | 结果显示 | 清理统计（批注移除数、控件解包数） |
...
- **输入支持**：单文件和文件夹两种模式
- **输出目录**：默认为 `文档/DocuFiller输出/清理`，用户可自定义
- **文件格式**：仅支持 .docx 格式
...
提供 JSON ↔ Excel 格式转换功能，方便用户在不同数据格式之间切换。

#### 转换方向

**JSON → Excel**：

- 读取 JSON 文件中的 `keywords` 数组
- 提取每个关键词项的 `key` 和 `value` 字段
- 生成两列格式的 Excel 文件（关键词 | 值），无表头
- 支持批量转换多个 JSON 文件
...
| 页面 | 区域 | 功能描述 |
|------|------|----------|
| 主界面 | 工具区域 | JSON 转 Excel 功能入口 |

#### 业务规则

- **输入格式**：JSON 文件必须包含 `keywords` 数组
- **输出格式**：生成无表头的两列 Excel（.xlsx）
- **批量转换**：支持同时转换多个 JSON 文件到指定目录
- **文件命名**：输出文件名与源 JSON 文件同名，扩展名改为 .xlsx
- **转换统计**：报告成功和失败数量，失败时提供错误信息
...
| 数据配置 | 数据配置区域 | 数据文件选择按钮，数据类型显示（Excel/JSON），数据预览表格，字段映射显示，数据验证状态指示器 |
| 输出设置 | 输出设置区域 | 输出文件夹拖拽区域，时间戳文件夹名称预览，输出路径显示 |
| 处理控制 | 处理控制区域 | 开始处理按钮（绿色大尺寸），暂停/恢复按钮，取消按钮，进度条，当前处理文件显示 |
...
| 区域 | UI 元素 |
|------|---------|
| 文件输入 | 拖拽区域（支持单文件和文件夹），文件列表（显示文件名、大小、状态） |
| 输出设置 | 输出目录选择，默认目录预览 |
| 处理控制 | 开始清理按钮，进度条 |
| 结果展示 | 清理统计（移除批注数、解包控件数），状态列表 |
...
| 数据源类型 | 关键词格式 | 匹配规则 |
|------------|------------|----------|
| JSON | 字段名（如 `FieldA`） | Tag 值 = JSON 字段名 |
...
| `JsonKeywordItem` | JSON 关键词项（键名、值、来源文件） |
